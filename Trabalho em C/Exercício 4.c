...
int main()
{
	...
	{
		abertura();
		printf("VOÇE ESTÁ CADASTRANDO A %d FRUTA DE %d\n\n", i + 1, n);
		fflush(stdin);
	...
	printf("\n\nDeseja vizualizar alguma faixa de preço específica? (s/n): ");
	scanf(" %c", &resp);

	if (resp == 's')
	{
		printf("\n\nQual faixa de preço que gostaria de ver??\n");
		...
		for (i = 0; i < n; i++)
		{
			if (cadastro[i].valor >= pmin && cadastro[i].valor <= pmax)
			{
				...
				{
					printf("O/a %s precisa de refrigeração? : NÃ£o\n", cadastro[i].nome);
				}
			}
		}
	}
	...
	for (i = 0; i < n; i++)
	{
		...
		if (res != NULL)
		{
			printf("\nOs nomes correspondentes são: %s\n", cadastro[i].nome);
			encontrado = true;
		}
	}
